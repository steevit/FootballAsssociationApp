...
public class Sedziowie extends JFrame {
	 ...
	 */
	public Sedziowie(Connection con1) throws ClassNotFoundException {
		con = con1;	
		...
		table.addMouseListener(new MouseAdapter() 
		{
			...
			public void mouseClicked(MouseEvent e) 
			{
				try 
				{
					...
					String query = 
							"select id_sedziego,imie,nazwisko,narodowosc,to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia from Sedziowie where id_sedziego='"+ID_+"'";
					...
					while(rs.next())
					{
						tID.setText(rs.getString("id_sedziego"));
		...
		bSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					String query = "{call P_SEDZIOWIE.P_DODAJ(?,?,?,?)}";
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					String query = "{call P_SEDZIOWIE.P_EDYTUJ("+tID.getText()+",'"+tName.getText()+"','"+tSurname.getText()+"','"+tNationality.getText()+"','"+java.sql.Date.valueOf(tBirthDate.getText())+"')}";
					CallableStatement cst = con.prepareCall(query);
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				...
				if(action==0)
				{
					try 
					{
						String query = "{call P_SEDZIOWIE.P_USUN("+tID.getText()+")}";
						CallableStatement cst = con.prepareCall(query);
		...
		tSearch.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyReleased(KeyEvent e) 
			{
				try 
				{
					...
					String query = "select id_sedziego, imie, nazwisko, narodowosc, to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia from Sedziowie where "+selection+"=? ";
					PreparedStatement pst = con.prepareStatement(query);
		...
		cBoxSelect.setModel(new DefaultComboBoxModel<String>(new String[] {"id_sedziego", "imie", "nazwisko", "narodowosc"}));
		...
		cBoxSort.setModel(new DefaultComboBoxModel<String>(new String[] {"id_sedziego", "imie", "nazwisko", "narodowosc","Data_urodzenia"}));
		...
		cBoxSort.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					...
					String query = "select id_sedziego, imie, nazwisko, narodowosc, to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia from Sedziowie ORDER BY "+selection;
					PreparedStatement pst = con.prepareStatement(query);
					ResultSet rs = pst.executeQuery();
					table.setModel(DbUtils.resultSetToTableModel(rs));
					pst.close();
	...
	public void refreshTable()
	{
		try 
		{
			String query = "select id_sedziego, imie, nazwisko, narodowosc, to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia from Sedziowie";
			PreparedStatement pst = con.prepareStatement(query);
			ResultSet rs = pst.executeQuery();
			table.setModel(DbUtils.resultSetToTableModel(rs));
			rs.close();
