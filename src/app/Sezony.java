...
public class Sezony extends JFrame {
	 ...
	 */
	public Sezony(Connection con1) throws ClassNotFoundException {
		con = con1;	
		...
		table.addMouseListener(new MouseAdapter() 
		{
			...
			public void mouseClicked(MouseEvent e) 
			{
				try 
				{
					...
					String query = 
							"select id_sezonu,rok from Sezony where id_sezonu='"+ID_+"'";
					...
					while(rs.next())
					{
						...
						tYear.setText(rs.getString("rok"));
					}
		...
		JLabel lYear = new JLabel("Rok");
		lYear.setBounds(10, 130, 100, 20);
		contentPane.add(lYear);
		
		tYear = new JTextField();
		tYear.setBounds(110, 130, 115, 20);
		...
		bSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					String query = "{call P_SEZONY.P_DODAJ("+tYear.getText()+")}";
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					String query = "{call P_SEZONY.P_EDYTUJ("+tID.getText()+","+tYear.getText()+")}";
					CallableStatement cst = con.prepareCall(query);
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				...
				if(action==0)
				{
					try 
					{
						String query = "{call P_SEZONY.P_USUN("+tID.getText()+")}";
						CallableStatement cst = con.prepareCall(query);
		...
		tSearch.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyReleased(KeyEvent e) 
			{
				try 
				{
					...
					String query = "select id_sezonu,rok from Sezony where "+selection+"=? ";
					PreparedStatement pst = con.prepareStatement(query);
		...
		cBoxSelect.setModel(new DefaultComboBoxModel<String>(new String[] {"id_sezonu", "rok"}));
		...
		cBoxSort.setModel(new DefaultComboBoxModel<String>(new String[] {"id_sezonu", "rok"}));
		...
		cBoxSort.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					...
					String query = "select id_sezonu,rok from Sezony ORDER BY "+selection;
					PreparedStatement pst = con.prepareStatement(query);
					ResultSet rs = pst.executeQuery();
					table.setModel(DbUtils.resultSetToTableModel(rs));
					pst.close();
	...
	public void refreshTable()
	{
		try 
		{
			String query = "select id_sezonu,rok from Sezony";
			PreparedStatement pst = con.prepareStatement(query);
			ResultSet rs = pst.executeQuery();
			table.setModel(DbUtils.resultSetToTableModel(rs));
			rs.close();
