...
public class CzyZdobylTytul extends JFrame {
	 ...
	 */
	public CzyZdobylTytul(Connection con1) throws ClassNotFoundException {
		con = con1;	
		...
		tSearch.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyReleased(KeyEvent e) 
			{
				try 
				{
					...
					String query = "select id_zawodnika,imie,nazwisko,CZY_ZDOBYL_TYTUL(id_zawodnika) AS Czy_zdobyl_tytul from Zawodnicy where "+selection+"=? ";
		...
		cBoxSort.setModel(new DefaultComboBoxModel<String>(new String[] {"id_zawodnika", "imie", "nazwisko", "Czy_zdobyl_tytul"}));
		...
		cBoxSort.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					...
					String query = "select id_zawodnika,imie,nazwisko,CZY_ZDOBYL_TYTUL(id_zawodnika) AS Czy_zdobyl_tytul from Zawodnicy ORDER BY "+selection;
					PreparedStatement pst = con.prepareStatement(query);
					ResultSet rs = pst.executeQuery();
					table.setModel(DbUtils.resultSetToTableModel(rs));
					pst.close();
	...
	public void refreshTable()
	{
		try 
		{
			String query = "select id_zawodnika,imie,nazwisko,CZY_ZDOBYL_TYTUL(id_zawodnika) AS Czy_zdobyl_tytul from Zawodnicy";
			PreparedStatement pst = con.prepareStatement(query);
			ResultSet rs = pst.executeQuery();
			table.setModel(DbUtils.resultSetToTableModel(rs));
			rs.close();
