...
public class Zawodnicy extends JFrame {
	 ...
	 */
	public Zawodnicy(Connection con1) throws ClassNotFoundException {	
		con = con1;
		...
		table.addMouseListener(new MouseAdapter() 
		{
			...
			public void mouseClicked(MouseEvent e) 
			{
				try 
				{
					...
					String query = 
							"select id_zawodnika, imie, nazwisko, narodowosc, to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia, id_druzyny from Zawodnicy where id_zawodnika='"+ID_+"'";
					...
					while(rs.next())
					{
						tID.setText(rs.getString("id_zawodnika"));
						...
						tTeam.setText(rs.getString("id_druzyny"));
					}
		...
		JLabel lTeam = new JLabel("ID Dru¿yny");
		lTeam.setBounds(10, 250, 100, 20);
		contentPane.add(lTeam);
		
		tTeam = new JTextField();
		tTeam.setBounds(110, 250, 115, 20);
		...
		bSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					String query = "{call P_ZAWODNICY.P_DODAJ(?,?,?,?,?)}";
					...
					cst.setInt(5, Integer.parseInt(tTeam.getText()));
					cst.execute();
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					String query = "{call P_ZAWODNICY.P_EDYTUJ("+Integer.parseInt(tID.getText())+",'"+tName.getText()+"','"+tSurname.getText()+"','"+tNationality.getText()+"','"+java.sql.Date.valueOf(tBirthDate.getText())+"',"+Integer.parseInt(tTeam.getText())+")}";
					CallableStatement cst = con.prepareCall(query);
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				...
				if(action==0)
				{
					try 
					{
						String query = "{call P_ZAWODNICY.P_USUN("+tID.getText()+")}";
						CallableStatement cst = con.prepareCall(query);
		...
		tSearch.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyReleased(KeyEvent e) 
			{
				try 
				{
					...
					String query = "select id_zawodnika, imie, nazwisko, narodowosc, to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia, z.id_druzyny, nazwa from Zawodnicy z, Druzyny d where z.id_druzyny=d.id_druzyny AND "+selection+"=? ";
					PreparedStatement pst = con.prepareStatement(query);
		...
		cBoxSelect.setModel(new DefaultComboBoxModel<String>(new String[] {"id_zawodnika", "imie", "nazwisko", "narodowosc","z.id_druzyny", "nazwa"}));
		...
		cBoxSort.setModel(new DefaultComboBoxModel<String>(new String[] {"id_zawodnika", "imie", "nazwisko", "narodowosc","Data_urodzenia","id_druzyny", "nazwa"}));
		...
		cBoxSort.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					...
					String query = "select id_zawodnika, imie, nazwisko, narodowosc, to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia, z.id_druzyny AS id_druzyny, nazwa from Zawodnicy z, Druzyny d where z.id_druzyny=d.id_druzyny ORDER BY "+selection;
					PreparedStatement pst = con.prepareStatement(query);
					ResultSet rs = pst.executeQuery();
					table.setModel(DbUtils.resultSetToTableModel(rs));
					pst.close();
	...
	public void refreshTable()
	{
		try 
		{
			String query = "select id_zawodnika, imie, nazwisko, narodowosc, to_char(data_ur, 'YYYY-MM-DD') AS Data_urodzenia, z.id_druzyny, nazwa from Zawodnicy z, Druzyny d where z.id_druzyny=d.id_druzyny ";
			PreparedStatement pst = con.prepareStatement(query);
			ResultSet rs = pst.executeQuery();
			table.setModel(DbUtils.resultSetToTableModel(rs));
			rs.close();
