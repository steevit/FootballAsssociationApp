...
public class IleMeczy extends JFrame {
	 ...
	 */
	public IleMeczy(Connection con1) throws ClassNotFoundException {
		con = con1;	
		...
		tSearch.addKeyListener(new KeyAdapter() 
		{
			...
			public void keyReleased(KeyEvent e) 
			{
				try 
				{
					...
					String query = "select id_druzyny,nazwa,ILE_MECZY(id_druzyny) AS Liczba_meczy from Druzyny where "+selection+"=? ";
		...
		cBoxSelect.setModel(new DefaultComboBoxModel<String>(new String[] {"id_druzyny", "nazwa"}));
		...
		cBoxSort.setModel(new DefaultComboBoxModel<String>(new String[] {"id_druzyny", "nazwa", "Liczba_meczy"}));
		...
		cBoxSort.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				try 
				{
					...
					String query = "select id_druzyny,nazwa,ILE_MECZY(id_druzyny) AS Liczba_meczy from Druzyny ORDER BY "+selection;
					PreparedStatement pst = con.prepareStatement(query);
					ResultSet rs = pst.executeQuery();
					table.setModel(DbUtils.resultSetToTableModel(rs));
					pst.close();
	...
	public void refreshTable()
	{
		try 
		{
			String query = "select id_druzyny,nazwa,ILE_MECZY(id_druzyny) AS Liczba_meczy from Druzyny";
			PreparedStatement pst = con.prepareStatement(query);
			ResultSet rs = pst.executeQuery();
			table.setModel(DbUtils.resultSetToTableModel(rs));
			rs.close();
